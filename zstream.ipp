...
deflatestream<Ct,Vt,sz>::deflatestream(Ct *c, stream<Ct,Vt,sz> *s, stream<Ct,Vt,sz> *t)
...
template <typename Ct, typename Vt, int sz>
deflatestream<Ct,Vt,sz>::~deflatestream() {
    if (_ok) {
        deflateEnd(&_strm);
    }
    _ok = false;
...
template <typename Ct, typename Vt, int sz>
int deflatestream<Ct,Vt,sz>::process(Ct const * const cfg, int len, sizebounded<Vt,sz> & inbuf) const {
    if (! _ok) {
...
template <typename Ct, typename Vt, int sz>
inflatestream<Ct,Vt,sz>::inflatestream(Ct *c, stream<Ct,Vt,sz> *s, stream<Ct,Vt,sz> *t)
    : stream<Ct,Vt,sz>(c,s,t)
{
  _strm.zalloc = Z_NULL;
  _strm.zfree = Z_NULL;
...
template <typename Ct, typename Vt, int sz>
inflatestream<Ct,Vt,sz>::~inflatestream() {
    if (_ok) {
        inflateEnd(&_strm);
    }
    _ok = false;
...
template <typename Ct, typename Vt, int sz>
int inflatestream<Ct,Vt,sz>::process(Ct const * const cfg, int len, sizebounded<Vt,sz> & inbuf) const {
    if (! _ok) { return 0; }
    _strm.avail_in = len;
    _strm.next_in = (unsigned char*)inbuf.ptr();
    _strm.avail_out = _buf.size();
...
template <typename Ct, typename Vt, int sz>
void inflatestream<Ct,Vt,sz>::push(int len, sizebounded<Vt,sz>& buf) const {
    sizebounded<Vt,sz> orig {buf};
    stream<Ct,Vt,sz>::push(len, buf);
    while (_strm.avail_in > 0) {
        std::memcpy((void*)buf.ptr(), orig.ptr() + (len - _strm.avail_in), _strm.avail_in);
        stream<Ct,Vt,sz>::push(_strm.avail_in, buf);
    }
}
