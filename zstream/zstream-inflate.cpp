...
/*
 * compile: g++ -o zinflate zstream-inflate.cpp -std=c++11 -g -I . -I ../ext/sizebounded -lz
 *
 *
 */
...
int main (int argc, char **argv)
{
    ...
    std::cout << "in total read " << ntot << " bytes and uncompressed to " << ncomp << " bytes." << std::endl;
...
stream<configuration,char,sz>* prepare_stream(int & ncomp, FILE *fout)
{
    ...
    sinfo->processor([&ncomp,fout](configuration const * const _c, int _l, sizebounded<char,sz>& _b)->int {
            ncomp += _l;
            std::cout << " uncompressed = " << ncomp << " bytes." << std::endl;
            if (_l > 0) {
    ...
    auto *inflater = new inflatestream<configuration,char,sz>(nullptr, nullptr, sinfo);
    return inflater;
}
